defmodule AuthexEmailConfirmation.Phoenix.ConfirmationControllerTest do
  ...
  describe "show/2" do
    ...
    test "confirms with valid token", %{conn: conn} do
      conn = get conn, Routes.authex_email_confirmation_confirmation_path(conn, :show, @valid_token)
    ...
    test "fails with invalid token", %{conn: conn} do
      conn = get conn, Routes.authex_email_confirmation_confirmation_path(conn, :show, @invalid_params)
      assert redirected_to(conn) == Routes.authex_registration_path(conn, :edit)
