defmodule Pow.Store.Backend.MnesiaCacheTest do
  ...
  describe "distributed nodes" do
    ...
    test "will join cluster" do
      ...
      expected_msg = "[Pow.Store.Backend.MnesiaCache] Mnesia cluster initiated on #{inspect node_a}"
      ...
      expected_msg = "[Pow.Store.Backend.MnesiaCache] Joined mnesia cluster nodes [#{inspect node_a}] for #{inspect node_b}"
      ...
      expected_msg = "[Pow.Store.Backend.MnesiaCache] Joined mnesia cluster nodes [#{inspect node_b}] for #{inspect node_a}"
      assert_receive {:log, ^node_a, :info, ^expected_msg}, @assertion_timeout
      assert :rpc.call(node_b, :mnesia, :system_info, [:running_db_nodes]) == [node_a, node_b]
      assert :rpc.call(node_b, MnesiaCache, :get, [config, "short_ttl_key_2_set_on_b"]) == "value"
    ...
    test "when init fails" do
      ...
      assert_receive {:log, _node, :error, "[Pow.Store.Backend.MnesiaCache] Couldn't join mnesia cluster because: {:aborted, {:no_exists, {Pow.Store.Backend.MnesiaCache, :cstruct}}}"}, @assertion_timeout
    end
