defmodule Authex.Plug.Base do
  @moduledoc """
  This plug macro will set the :authex_config key, and attempt to fetch and assign
  a user in the connection, if it has not already been assigned.

  You can use this to build your own authex plug:
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def call(conn, config) do
        config = Authex.Config.put(config, :mod, __MODULE__)
        conn   = Authex.Plug.put_config(conn, config)

        conn
        |> Authex.Plug.current_user()
      ...
      defp fetch_config(conn),
        do: Authex.Plug.fetch_config(conn)

      defp assign_user(conn, user, config),
        do: Authex.Plug.assign_current_user(conn, user, config)
